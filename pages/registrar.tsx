...
import { fileToArrayBuffer, sha256ArrayBuffer } from '../lib/hash';
...
export default function RegistrarPage() {
  ...
  try {
    ...
    alert('Commit enviado (demo) y agregado al Feed');
    // redirigir al feed
  ...
  return (
    <div className="min-h-screen bg-[#0b1220] text-slate-100">
      ...
      <main className="max-w-4xl mx-auto px-4 py-10">
        ...
        <form onSubmit={onSubmit} className="bg-[#121a2b] border border-white/10 rounded-2xl p-6 grid gap-5">
          {/* Evidencia */}
          ...
          <div className="grid gap-2">
            ...
            <div className="flex gap-2">
              <input className="flex-1 bg-[#0e1627] border border-white/10 rounded px-3 py-2"
                placeholder="Ej: -16.5000, -68.1500 o Av. Principal, La Paz"
                value={location} onChange={e=>setLocation(e.target.value)} />
              <button type="button" onClick={useCurrentLocation}
                className="px-3 py-2 rounded bg-white/10 hover:bg-white/15">Usar mi ubicación</button>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="grid gap-2">
              ...
              <select className="bg-[#0e1627] border border-white/10 rounded px-3 py-2"
                value={type} onChange={e=>setType(e.target.value)}>
                <option value="">Selecciona el tipo</option>
                <option>Colisión</option><option>Exceso de velocidad</option>
                <option>Estacionamiento indebido</option><option>Semáforo en rojo</option>
            ...
            <div className="grid gap-2">
              ...
              <input className="bg-[#0e1627] border border-white/10 rounded px-3 py-2"
                placeholder="ABC-1234" value={plate} onChange={e=>setPlate(e.target.value)} />
            </div>
          </div>
          ...
          <div className="grid gap-2">
            ...
            <textarea className="bg-[#0e1627] border border-white/10 rounded px-3 py-2 h-28"
              placeholder="Describe lo ocurrido…" value={notes} onChange={e=>setNotes(e.target.value)} />
          </div>

          <div className="rounded-xl border border-blue-500/40 bg-blue-500/10 p-4 text-blue-200 text-sm">
            Al sellar esta evidencia, se creará un hash de compromiso en la blockchain. Deberás revelar la evidencia completa posteriormente.
          </div>
