...
const demoDisputas = [
  { id: "INC-2025-002", tipo: "Exceso de velocidad", placa: "XYZ-5678", razon: "Error en medición", prioridad: "Alta" as const },
  { id: "INC-2025-008", tipo: "Colisión", placa: "MNO-2468", razon: "Responsabilidad disputada", prioridad: "Media" as const },
];

const demoPendientes = [
  { id: "INC-2025-010", tipo: "Semáforo en rojo", placa: "ABC-1111" },
];
...
function Pill({
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs ${map[color]}`}>
      {children}
    </span>
  );
}
...
export default function AuditorPage() {
  ...
  const disputasLocal = useMemo(
    () => local.filter((i) => i.state === "disputed"),
    [local]
  );
  const pendientesLocal = useMemo(
    () => local.filter((i) => i.state === "pending"),
    [local]
  );
...
* Este incidente es de demo y no está en tu almacenamiento local.`
  ...
  /** RESOLVER: cambia estado si existe local */
  function resolveIncident(id: string) {
    const it = local.find((i) => i.id === id);
    if (!it) {
      alert(
        "Este incidente es de demo. Para resolverlo, primero impórtalo o registra uno real."
      );
      return;
    }
    const next = it.state === "disputed" ? "resolved" : "revealed";
    updateIncident(id, { state: next as any });
    alert(`Incidente ${id} marcado como ${next.toUpperCase()}`);
  }
            ...
            <div className="bg-gradient-to-b from-[#111a2e] to-[#0c1426] border border-white/10 rounded-2xl p-6 shadow-[0_6px_30px_-15px_rgba(0,0,0,.6)]">
              ...
              <div className="flex items-center gap-2 text-amber-900/90 bg-amber-200/60 border border-amber-300/50 rounded-xl px-3 py-2 mb-4">
                ...
                <span className="text-sm">
                  Los incidentes deben ser revelados dentro del plazo configurado. Después del deadline,
                  pueden ser marcados como <b>"stale"</b>.
                </span>
              </div>
                        ...
                        <tr key={p.id} className="border-t border-white/10">
                          ...
                          <td className="py-2">
                            <div className="flex gap-2">
                              ...
                              <button
                                onClick={() =>
                                  updateIncident(p.id, { state: "stale" as const })
                                }
          ...
          <section className="bg-gradient-to-b from-[#111a2e] to-[#0c1426] border border-white/10 rounded-2xl p-6 grid gap-6 shadow-[0_6px_30px_-15px_rgba(0,0,0,.6)]">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs mt-2">
                Tiempo límite desde el commit hasta el reveal. Recomendado: 24–48
                horas.
              </p>
            </div>
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          ...
                          alert(
                            `${card.title}: ${
                              (s.roles as any)[card.key] ? "Activo" : "Inactivo"
                            }`
                          );
                        }}
