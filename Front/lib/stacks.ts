...
export const NETWORK =
  'https://explorer.hiro.so/txid/0xdf721bedee37afc92046c9d534f128cd6892d4dab6eecd55078a25ed07d3792e?chain=mainnet';
...
export function openDispute(id: number, reasonHash: string) {
  return makeContractCall({
    contractAddress: CONTRACT_ADDRESS,
    contractName: CONTRACT_NAME,
    functionName: 'open-dispute',
    functionArgs: [uintCV(id), bufferCV(Buffer.from(reasonHash.slice(2), 'hex'))],
    network: NETWORK,
    appDetails: { name: 'SafeTraffic Ledger', icon: '/logo.png' },
    postConditionMode: 1,
...
export function resolveDispute(id: number, newStatus: number, senderAddress: string) {
  return makeContractCall({
    contractAddress: CONTRACT_ADDRESS,
    contractName: CONTRACT_NAME,
    functionName: 'resolve-dispute',
    functionArgs: [uintCV(id), uintCV(newStatus)],
    network: NETWORK,
    appDetails: { name: 'SafeTraffic Ledger', icon: '/logo.png' },
    postConditionMode: 1,
    stxAddress: senderAddress,
